...
CoolAST parse_ast(bh_str* str)
{
    ...
    AST.classes = malloc(class_count * sizeof(CoolClass));

    for (int i = 0; i < class_count; i++)
    {
        AST.classes[i] = parse_class(str);
    }
...
CoolClass parse_class(bh_str* str)
{
    ...
    class.features = malloc(class.feature_count * sizeof(CoolFeature));

    for (int i = 0; i < class.feature_count; i++)
    {
        class.features[i] = parse_feature(str);
    }
...
CoolFeature parse_feature(bh_str* str)
{
    ...
    if (bh_str_equal(feature_name_str, bh_str_from_cstr("method")))
    {
        ...
        feature.formals = malloc(formal_count * sizeof(CoolFormal));
    ...
    if (bh_str_equal(feature_name_str, bh_str_from_cstr("attribute_init"))) feature.body = parse_expression(str);
    else if (bh_str_equal(feature_name_str, bh_str_from_cstr("method"))) feature.body = parse_expression(str);
...
CoolExpression parse_expression(bh_str* str)
{
    ...
    case COOL_EXPR_TYPE_ASSIGN:
        ...
        expression.data.assign.rhs = malloc(sizeof(CoolExpression));
        *expression.data.assign.rhs = parse_expression(str);
        break;
    case COOL_EXPR_TYPE_DYNAMIC_DISPATCH:
    case COOL_EXPR_TYPE_STATIC_DISPATCH:
        expression.data.dynamic_dispatch.e = malloc(sizeof(CoolExpression));
        *expression.data.dynamic_dispatch.e = parse_expression(str);
    case COOL_EXPR_TYPE_SELF_DISPATCH:
        ...
        expression.data.self_dispatch.args = malloc(expression.data.self_dispatch.args_length * sizeof(CoolExpression));
        for (int i = 0; i < expression.data.self_dispatch.args_length; i++)
        {
            expression.data.self_dispatch.args[i] = parse_expression(str);
        }
        break;
    case COOL_EXPR_TYPE_IF:
        expression.data.if_expr.predicate = malloc(sizeof(CoolExpression));
        *expression.data.if_expr.predicate = parse_expression(str);
        expression.data.if_expr.then_branch = malloc(sizeof(CoolExpression));
        *expression.data.if_expr.then_branch = parse_expression(str);
        expression.data.if_expr.else_branch = malloc(sizeof(CoolExpression));
        *expression.data.if_expr.else_branch = parse_expression(str);
        break;
    case COOL_EXPR_TYPE_WHILE:
        expression.data.while_expr.predicate = malloc(sizeof(CoolExpression));
        *expression.data.while_expr.predicate = parse_expression(str);
        expression.data.while_expr.body = malloc(sizeof(CoolExpression));
        *expression.data.while_expr.body = parse_expression(str);
        break;
    case COOL_EXPR_TYPE_BLOCK:
        {
            bh_str num_str = eat_until_newline(str);
            expression.data.block.body_length = uint_from_str(num_str);
            expression.data.block.body = malloc(sizeof(CoolExpression) * expression.data.block.body_length);
            for (int i = 0; i < expression.data.block.body_length; i++)
            {
                expression.data.block.body[i] = parse_expression(str);
            }
            break;
        }
    ...
    case COOL_EXPR_TYPE_ISVOID:
        expression.data.isvoid.e = malloc(sizeof(CoolExpression));
        *expression.data.isvoid.e = parse_expression(str);
        break;
    ...
    case COOL_EXPR_TYPE_EQ:
        expression.data.binary.x = malloc(sizeof(CoolExpression));
        *expression.data.binary.x = parse_expression(str);
        expression.data.binary.y = malloc(sizeof(CoolExpression));
        *expression.data.binary.y = parse_expression(str);
        break;
    case COOL_EXPR_TYPE_NOT:
    case COOL_EXPR_TYPE_NEGATE:
        expression.data.unary.x = malloc(sizeof(CoolExpression));
        *expression.data.unary.x = parse_expression(str);
        break;
    ...
    case COOL_EXPR_TYPE_LET:
        ...
        expression.data.let.bindings = malloc(expression.data.let.binding_count * sizeof(CoolLetBinding));
        for (int i = 0; i < expression.data.let.binding_count; i++)
        {
            ...
            {
                expression.data.let.bindings[i].exp = malloc(sizeof(CoolExpression));
                *expression.data.let.bindings[i].exp = parse_expression(str);
            }
        }
        expression.data.let.expr = malloc(sizeof(CoolExpression));
        *expression.data.let.expr = parse_expression(str);
        break;
    case COOL_EXPR_TYPE_CASE:
        expression.data.case_expr.expr = malloc(sizeof(CoolExpression));
        *expression.data.case_expr.expr = parse_expression(str);
        expression.data.case_expr.element_count = eat_uint_until_newline(str);
        expression.data.case_expr.elements = malloc(expression.data.case_expr.element_count * sizeof(CoolCaseElement));
        for (int i = 0; i < expression.data.case_expr.element_count; i++)
        {
            ...
            expression.data.case_expr.elements[i].body = malloc(sizeof(CoolExpression));
            *expression.data.case_expr.elements[i].body = parse_expression(str);
        }
        break;
    default:
